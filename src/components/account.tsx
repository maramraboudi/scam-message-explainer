...
export function Account({ user, onLogin, onLogout }: { user: UserSession | null; onLogin: (user: UserSession) => void; onLogout: () => void }) {
  ...
  if (user) return <div className="view account-view animate-fade-up"><div className="account-card signed-in"><div className="account-avatar">{user.name.slice(0, 2).toUpperCase()}</div><h1>{user.name}</h1><p>{user.email}</p><span><CheckCircle2 size={16} />Workspace session active</span><div className="account-details"><div><small>Authentication</small><strong>Secure demo session</strong></div><div><small>Workspace role</small><strong>Security analyst</strong></div><div><small>Multi-factor authentication</small><strong>Recommended</strong></div></div><button className="ghost" onClick={onLogout}>Sign out</button></div></div>;
  return (
    <div className="view account-view animate-fade-up">
      <div className="auth-layout">
        <div className="auth-message"><ShieldCheck size={36} /><h1>Your investigations, secured.</h1><p>Sign in to keep incident history, bookmarks, reports, and team settings connected to your workspace.</p><ul><li><LockKeyhole size={16} />Private workspace boundaries</li><li><KeyRound size={16} />Session protection and access controls</li><li><CheckCircle2 size={16} />Audit-ready account activity</li></ul></div>
        <form className="auth-form" onSubmit={submit}>
          ...
          <h2>{mode === "login" ? "Welcome back" : "Create your workspace"}</h2><p>{mode === "login" ? "Continue to your security dashboard." : "Start with a free Community account."}</p>
          ...
          <small>This local portfolio build demonstrates the account workflow. Connect a managed identity provider before handling production users.</small>
        </form>
      </div>
    </div>
  );
